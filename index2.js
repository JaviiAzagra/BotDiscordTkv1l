...
console.log(client.db.get("1092792648949047378"));
// comandos
...
client.on("messageCreate", async (message) => {
  ...
  if (command === "setup") {
    ...
    const msg = await channel.send({
      ...
      components: [new Discord.MessageActionRow().addComponents([new Discord.MessageButton().setStyle("SUCCESS").setLabel("Soporte").setEmoji("🎫").setCustomId("crearticket"),
      ([new Discord.MessageButton().setStyle("DANGER").setLabel("Postulación").setEmoji("👺").setCustomId("postular")]),
      ([new Discord.MessageButton().setStyle("PRIMARY").setLabel("Parceira").setEmoji("👥").setCustomId("parceira")]),
      ([new Discord.MessageButton().setStyle("SECONDARY").setLabel("Solicitar Match").setEmoji("✨").setCustomId("solicitarmatch")])])]
    ...
    const avatar = await message.channel.send({
      embeds: [
        new Discord.MessageEmbed()
          ...
          .setFooter(`Pedido por el SIMP de oscar `),
      ],
    });
  }
  ...
  if (command === "play") {
    if (!args.length)
      return message.reply(`❌ **Tienes que especificar el nombre de la cancion!**`);
    if (!message.member.voice?.channel)
      return message.reply(`❌ **Tienes que estar en une canal de voz para poder ejecutar este comando**`);
    if (message.guild.me.voice?.channel && message.member.voice?.channel.id != message.guild.me.voice?.channel.id)
      return message.reply(`❌ **Tienes que estar en el mismo canal de voz que ${client.user.tag} para poder ejecutar este comando**`);
  ...
  if (command === "skip") {
    ...
    if (!message.member.voice?.channel)
      return message.reply(`❌ **Tienes que estar en un canal de voz para poder ejecutar este comando!**`);
    if (message.guild.me.voice?.channel && message.member.voice?.channel.id != message.guild.me.voice?.channel.id)
      return message.reply(`❌ **Tienes que estar en el mismo canal de voz que ${client.user.tag} pra poder ejecutar este comando!**`);
    ...
    switch (interaction.customId) {
      ...
      case "postular": {
        if (client.tickets.has(interaction.member.user.id)) {
          ...
          if (
            ticket &&
            client.tickets.get(interaction.member.user.id, "closed") ==
            false
          )
            return interaction.reply({
              content: `❌Ya tienes un ticket creado en <#${ticket.id}>`,
        ...
        channel.send({
          embeds: [
            new Discord.MessageEmbed()
              ...
              .setDescription(
                `Bienvenido a postulaciones ${interaction.member}\n**Para postular a 1NFINYT1 rellena la plantilla que se adjuntará en unos instantes.**`
              )
              .setColor("BLUE")
              .setTimestamp(),
          ],
        ...
        channel.send({
          embeds: [
            new Discord.MessageEmbed()
              ...
              .setDescription(
                `Bienvenido a **PARCEIRA** ${interaction.member}\n**Para solicitar una parceira rellena la plantilla que se adjuntará en unos instantes.**`
              )
              .setColor("BLUE")
              .setTimestamp(),
          ],
      ...
      case "solicitarmatch": {
        ...
        channel.send({
          embeds: [
            new Discord.MessageEmbed()
              .setTitle(`Ticket para **Solicitar Match** ${interaction.member.user.tag}`)
              .setDescription(
                `Bienvenido a **Solicitar Match** ${interaction.member}\n**Para solicitar una match rellena la plantilla que se adjuntará en unos instantes.**`
              )
              .setColor("BLUE")
              .setTimestamp(),
          ],
...
client.on("guildMemberAdd", async (member) => {
  ...
  try {
    ...
    if (data) {
      ...
      const attachment = new Discord.MessageAttachment('https://imgur.com/UwI6wcx.png')
